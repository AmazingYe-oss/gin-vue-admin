...
type ExaCustomer struct {
	...
	DeptId             uint           `json:"deptId" form:"deptId" gorm:"column:dept_id;comment:归属部门ID(数据权限)"`       // 归属部门(数据权限引擎自动盖章/过滤)
	CreatedBy          uint           `json:"createdBy" form:"createdBy" gorm:"column:created_by;comment:创建人(数据权限)"` // 创建人(数据权限引擎自动盖章/过滤)
